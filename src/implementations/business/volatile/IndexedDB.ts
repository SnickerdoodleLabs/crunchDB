...
export class IndexedDB {
      ...
      try {
        ...
        request.onupgradeneeded = (event: IDBVersionChangeEvent) => {
          ...
          existingObjectStores.forEach(storeName => {
            if (
              !this.schema.find(tableDetails => tableDetails.name === storeName)
            ) {
              this.logUtils.debug(
  ...
  private migrateStore(storeName: ERecordKey, db: IDBDatabase) {
    ...
    const currentMigrator = this.schema.find(st => st.name == storeName)
      ?.migrator;
    ...
    tx.oncomplete = _event => {
      if (migrationNeeded) {
        this.logUtils.debug(
          `Migration function took ${(this.timeUtils.getMillisecondNow() -
            startingTime) /
            1000} seconds for store ${storeName}`
        );
      }
          ...
          new Promise((resolve, reject) => {
            ...
            getCursor.onsuccess = _event => {
              ...
              if (cursor) {
                ...
                const versionedData: VersionedObject = currentMigrator.getCurrent(
                  data,
                  version
                );
                if (versionedData) {
